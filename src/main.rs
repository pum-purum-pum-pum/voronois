...
use na::{Point3, Point2, Isometry2};
use utils::KissMe;
...
pub fn check_in(polygon: &[Point2<f32>], point: Point2<f32>) -> bool {
	...
	for segment in poly_segments.iter() {
		...
		if inner_vec.perp(&side_vec) > -EPS { // it's float's ">="
			plus_check_sum += 1;
		}
...
pub fn check_on(polygon: &[Point2<f32>], point: Point2<f32>) -> bool {
	...
	for segment in poly_segments.iter() {
		let inner_vec = point.coords - segment.0.coords;
		let side_vec = segment.1.coords - segment.0.coords;
		if inner_vec.perp(&side_vec).abs() < EPS { // it's float's ">="
			flag = true;
		}
	}
	flag
...
fn segment_intersection(s1: &Segment<f32>, s2: &Segment<f32>) -> Option<Point2<f32>> {
	// let dir1 = s1.b().coords - s1.a().coords;
	// let dir2 = s2.b().coords - s2.a().coords;
	// let toi1 = s2.toi_with_ray(&Isometry2::identity(), &Ray::new(*s1.a(), dir1), true);
	// let toi2 = s1.toi_with_ray(&Isometry2::identity(), &Ray::new(*s2.a(), dir2), true);
	// if let (Some(toi1), Some(toi2)) = (toi1, toi2) {
	// 	if toi1 < dir1.norm() + std::f32::EPSILON && toi2 < dir2.norm() + std::f32::EPSILON {
	// 		Some(s1.a() + toi1 * dir1)
	// 	} else {
	// 		None
	// 	}
	...
	// assert!(dir.norm() > EPS);
	// assert!((s2.b().coords - s2.a().coords).norm() > EPS);
	...
	if let (Some(toi1), Some(toi2)) = (toi1, toi2) {
		...
		// debug_assert!(check_in_segment(res, s1.clone()));
		// debug_assert!(check_in_segment(res, s2.clone()));
...
fn main() {
    ...
    let mut colored_segments: 
    	Vec<(Vec<(Point2<f32>, Point2<f32>)>, Point3<f32>)> = vec![];
	...
	while !kiss_me.window.should_close()  {
		for event in kiss_me.window.events().iter() {
	        match event.value {
	        	WindowEvent::CursorPos(x, y, _modif) => {
				    ...
				    let (vor_polys, vor_pts, clip_points) = destruction(
				    	&bounding_poly, 
				  ...
				  //   }
				  	for poly in vor_polys.iter() {
				  		let current_color = Point3::new(rng.gen_range(0.1, 1.0), rng.gen_range(0.1, 1.0), rng.gen_range(0.1, 1.0));
				  		let cur_segments = to_segments(poly);
		...
		kiss_me.window.draw_point(&Point3::new(mouse.x, mouse.y, 0.0), &Point3::new(0.0, 0.0, 0.0));
		kiss_me.render();
	}
}
